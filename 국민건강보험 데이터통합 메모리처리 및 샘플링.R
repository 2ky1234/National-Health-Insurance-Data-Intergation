...
# 코드 용도 : 통합
# 코드 최종 결과물 : 통합코드 
# 코드 작성자 : 김예린, 박채영, 소은비, 이근영
# 코드 작성 일자 : 2019.4.9
# 업데이트 내용 : 2019/4/1일 에러코드 수정, 샘플링 방법1
# 메모리 최적화 코딩 유무 : Y
# 총 실행시간 산출 코드 유뮤 : Y
...
##### 샘플링
############### JK에 고액 진료비환자 정의(10%기준)
#T120+T220
T20 <- rbind(T120_02,T220_02)
T20 <- subset(T20,select=c(PERSON_ID,EDEC_TRAMT))
#PERSON_ID별 총액 산출
T20_PERSON <- T20 %>% group_by(PERSON_ID) %>% summarise(SUM_AMT=sum(EDEC_TRAMT)) %>% arrange(desc(SUM_AMT))
#PERCENTAGE 표시 및 상위 10퍼센트 고액환자로 구분
T20_PERSON$NUM <- c(nrow(T20_PERSON):1)
T20_PERSON$PERCEN <- T20_PERSON$NUM/nrow(T20_PERSON)*100
T20_PERSON <-  T20_PERSON %>% mutate(CLASS_LABEL=ifelse(PERCEN>=90,'1','0'))
T20_PERSON <- subset(T20_PERSON, select = c(PERSON_ID, SUM_AMT, CLASS_LABEL))

#JK+T20
JK20 <- left_join(JK_02, T20_PERSON, by="PERSON_ID")
JK20$SUM_AMT <- ifelse(is.na(JK20$SUM_AMT), 0, JK20$SUM_AMT)
JK20$CLASS_LABEL <- ifelse(is.na(JK20$CLASS_LABEL), 0, JK20$CLASS_LABEL)

########################고액진료비환자
############### 분포확인
JK20_HIGH <- JK20 %>% filter(JK20$CLASS_LABEL==1)
# 히스토그램 확인
#hist(JK20_HIGH$SUM_AMT)
# 구간 150개로 나눴을 때 히스토그램 확인
#hist(JK20_HIGH$SUM_AMT,breaks=150)

# 확률분포함수 그래프 그리기
#hist(JK20_HIGH$SUM_AMT, main = "", xlab = "Sample Means", prob = T, col = "gray")
#lines(density(JK20_HIGH$SUM_AMT), col = "darkblue", lwd = 2)

############### 고액 진료비환자에서 50퍼센트 샘플링
# 금액 기준으로 10개 구간(그룹)으로 나누기
JK20_HIGH$AMT_GROUP <- cut(JK20_HIGH$SUM_AMT, breaks = 10, labels = c(1:10))
table(JK20_HIGH$AMT_GROUP)

# 구간에 따라 층화 임의추출
size <- c()
for (i in 1:10){
  num <- ceiling(length(which(JK20_HIGH$AMT_GROUP==i))/2)
  if (num>0){
    size <- c(size,num)
  }
}
JK20_HIGH <- JK20_HIGH %>% arrange(JK20_HIGH$SUM_AMT)
set.seed(111)
sam <- strata(data=JK20_HIGH, stratanames=c("AMT_GROUP"),size=size, method="srswor",description=TRUE)
JK20_HIGH_SAMPLE <- getdata(JK20_HIGH,sam)

# 구간 150개로 나눴을 때 히스토그램 확인
#hist(JK20_HIGH$SUM_AMT,breaks=150)
#hist(JK20_HIGH_SAMPLE$SUM_AMT,breaks=150)

########################고액진료비환자X
############### 분포확인
JK20_NORMAL <- JK20 %>% filter(JK20$CLASS_LABEL==0)
# 히스토그램 확인
#hist(JK20_NORMAL$SUM_AMT)
# 구간 150개로 나눴을 때 히스토그램 확인
#hist(JK20_NORMAL$SUM_AMT,breaks=150)

# 확률분포함수 그래프 그리기
#hist(JK20_NORMAL$SUM_AMT, main = "", xlab = "Sample Means", prob = T, col = "gray")
#lines(density(JK20_NORMAL$SUM_AMT), col = "darkblue", lwd = 2)

############### 고액 진료비아닌환자에서 50퍼센트 샘플링
# 금액 기준으로 10개 구간(그룹)으로 나누기
JK20_NORMAL$AMT_GROUP <- cut(JK20_NORMAL$SUM_AMT, breaks = 10, labels = c(1:10))
table(JK20_NORMAL$AMT_GROUP)

# 구간에 따라 층화 임의추출
size <- c()
for (i in 1:10){
  num <- length(which(JK20_NORMAL$AMT_GROUP==i))/18
  if (num<1){num <- ceiling(num)}
  if (num>1){num <- round(num)}
  if (num!=0){
    size <- c(size,num)
  }
}
JK20_NORMAL <- JK20_NORMAL %>% arrange(JK20_NORMAL$SUM_AMT)
set.seed(222)
sam <- strata(data=JK20_NORMAL, stratanames=c("AMT_GROUP"),size=size, method="srswor",description=TRUE)
JK20_NORMAL_SAMPLE <- getdata(JK20_NORMAL,sam)

# 구간 150개로 나눴을 때 히스토그램 확인
#hist(JK20_NORMAL$SUM_AMT,breaks=150)
#hist(JK20_NORMAL_SAMPLE$SUM_AMT,breaks=150)

######### HIGH+NORMAL 분포확인
JK20_SAMPLE <- rbind(JK20_HIGH_SAMPLE,JK20_NORMAL_SAMPLE)
hist(JK20$SUM_AMT,breaks=150)
hist(JK20_SAMPLE$SUM_AMT,breaks=150)

group_id <- JK20_SAMPLE$PERSON_ID

JK_02 <- subset(JK_02, JK_02$PERSON_ID %in% group_id)
T120_02 <- subset(T120_02, T120_02$PERSON_ID %in% group_id)
T220_02 <- subset(T220_02, T220_02$PERSON_ID %in% group_id)

group_seq1 <- T120_02$KEY_SEQ
group_seq2 <- T220_02$KEY_SEQ
...
# 샘플링
T130_02 <- subset(T130_02, T130_02$KEY_SEQ %in% group_seq1)
...
# 샘플링
nhid_gy40_t1_2002 <- subset(nhid_gy40_t1_2002, nhid_gy40_t1_2002$KEY_SEQ %in% group_seq1)
...
# 샘플링
T160_02 <- subset(T160_02, T160_02$KEY_SEQ %in% group_seq1)

# 약제 파일 불러오기
ATC1 <- read_excel("./StuR/R code/ATC/ATC 1.xlsx")
ATC2 <- read_excel("./StuR/R code/ATC/ATC 2.xlsx")
ATC3 <- read_excel("./StuR/R code/ATC/ATC 3.xlsx")
ATC4 <- read_excel("./StuR/R code/ATC/ATC 4.xlsx")
### ATC 파일 전처리 ###
...
# 샘플링
T230_02 <- subset(T230_02, T230_02$KEY_SEQ %in% group_seq2)
...
# 샘플링
nhid_gy40_t2_2002 <- subset(nhid_gy40_t2_2002, nhid_gy40_t2_2002$KEY_SEQ %in% group_seq2)
...
# 샘플링
T260_02 <- subset(T260_02, T260_02$KEY_SEQ %in% group_seq2)
...
#####><###T30_02 NA처리#####
#**UN_COST NA있는 row 제거#
T30_02 <- na.omit(T30_02$UN_COST)

##### <>T30_02 KEY_SEQ
##**교수님 코드에서는 ITEM_CD,DD_MQTY_EXEC_FREQ,MDCN_EXEC_FREQ,DD_MQTY_FREQ도 삭제해야함
T30_02 <- subset(T30_02, select = -c(SEQ_NO,RECU_FR_DT,DD_MQTY_EXEC_FREQ,MDCN_EXEC_FREQ,DD_MQTY_FREQ))
...
quantile(high_cost_02$sum_amt,na.rm = TRUE)
...
high_ox <- high_ox %>% select(PERSON_ID,high_ox)
high_ox <- rename(high_ox, CLASS_LABEL=high_ox)

ALL <- merge(x = ALL, y = high_ox, by = 'PERSON_ID', all.x = TRUE)

#파일 저장 
setwd('C:/CohortDB/StuR/R code/')
write.csv(ALL,file = "TOTAL2002_190408.csv", row.names = FALSE)
...
#**교수님 파일에서는 삭제하지 말것
#ALL <- subset(ALL,select= -c(IN_PAT_CORS_TYPE, CTRB_PT_TYPE_CD, DFAB_PTN_CD))
...
#PPP YKIHO_CD_NEW 펼치기
# 필요한 변수만 select
ALL$YKIHO_CD_NEW <- as.character(ALL$YKIHO_CD_NEW)
ALL$YKIHO_CD_NEW <- paste("YKIHO_CD_NEW.",ALL$YKIHO_CD_NEW,sep='')
# Num 개수 세기
ALL <- ALL %>%  group_by(KEY_SEQ,YKIHO_CD_NEW) %>% mutate(ITEM_SUM = n())
#중복제거
ALL <- unique(ALL)
# 청구일련번호 별로 펼치기
ALL <- as.data.frame(spread(ALL,YKIHO_CD_NEW,ITEM_SUM,fill=0))

### PPP FORM_CD_1 기준으로 펼치기
# 필요한 변수만 select
ALL$FORM_CD_1 <- as.character(ALL$FORM_CD_1)
ALL$FORM_CD_1 <- paste("FORM_CD_1.",ALL$FORM_CD_1,sep='')
# Num 개수 세기
ALL <- ALL %>%  group_by(KEY_SEQ,FORM_CD_1) %>% mutate(ITEM_SUM = n())
#중복제거
ALL <- unique(ALL)
# 청구일련번호 별로 펼치기
ALL <- as.data.frame(spread(ALL,FORM_CD_1,ITEM_SUM,fill=0)) 

### PPP FORM_CD_2 기준으로 펼치기
# 필요한 변수만 select
ALL$FORM_CD_2 <- as.character(ALL$FORM_CD_2)
ALL$FORM_CD_2 <- paste("FORM_CD_2.",ALL$FORM_CD_2,sep='')
# Num 개수 세기
ALL <- ALL %>%  group_by(KEY_SEQ,FORM_CD_2) %>% mutate(ITEM_SUM = n())
#중복제거
ALL <- unique(ALL)
# 청구일련번호 별로 펼치기
ALL <- as.data.frame(spread(ALL,FORM_CD_2,ITEM_SUM,fill=0)) 
#PPP KEY_SEQ 삭제
...
#파일 저장 
setwd('C:/CohortDB/StuR/R code/')
write.csv(ALL,file = "TOTAL2002_190408_PERSON.csv", row.names = FALSE)
